...
| **Inquire Agent** | 从 GitHub/vulhub 获取 PoC，分析利用方法 | `ReadHTML` |
| **Exploit Agent** | 执行漏洞利用操作（命令行/浏览器自动化） | `EXECMD`, `ReadHTML`, `Playwright` |
...
│   ├── prompt.py                # Agent提示词模板（ReAct格式，动态生成）
│   ├── tools.py                 # Agent工具定义（EXECMD/ReadHTML/Playwright/ServicePort）
│   ├── terminal.py              # 命令执行（SSH远程 + 本地xray执行）
...
│   ├── result/                  # 测试结果输出目录
│   ├── finalbench.jsonl         # 完整基准数据（供pre模块使用）
│   └── test_*.py                # 测试脚本（连接测试/xray测试）
├── bench/                        # 测试基准数据集
│   ├── data.jsonl               # 20个CVE漏洞元数据
│   └── [各漏洞类型]/[CVE编号]/
│       └── docker-compose.yml   # 每个漏洞的Docker环境
├── pre/                          # 预处理模块
│   ├── pre.py                   # 预处理Agent（独立ReAct Agent，用于预实验）
│   └── terminal.py              # 预处理专用终端（SSH连接）
├── xray/                         # xray扫描器（需手动放置）
...
psm:
  ...
  exp_iterations: 5       # Exploit Agent单次最大工具调用数
  query_iterations: 2     # Inquire Agent单次最大工具调用数
  scan_iterations: 2      # Scan Agent单次最大工具调用数
