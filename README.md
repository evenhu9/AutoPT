...
- 🔍 **自动漏洞扫描**: 集成 nmap 全端口扫描 + xray 扫描器进行自动化漏洞发现，支持 LLM 驱动的智能端口发现
- 💡 **双层成功验证**: LLM 智能判断 + 正则表达式兜底，确保利用结果准确判定
- 🔧 **多工具集成**: 支持命令行工具（EXECMD）、HTML 内容获取（ReadHTML）、浏览器自动化（Playwright）、端口扫描（PortScan）
- 📊 **完整测试基准**: 包含20个真实CVE漏洞的Docker测试环境，涵盖 OWASP TOP 10 全部10个类别
- 🗜️ **上下文智能压缩**: 多层压缩机制（工具输出摘要 → 结构化上下文 → 服务指纹提取 → 失败引导生成 → ANSI过滤），避免 LLM 上下文溢出
- 📡 **LLM驱动端口发现**: 参考 pentest-agent 设计，LLM 自主生成 nmap 扫描命令并循环执行，智能发现靶机所有开放端口和服务
...
| 节点 | 功能 | 可用工具 |
|------|------|----------|
| **Scan Agent** | 使用 nmap 扫描全端口 + xray 对目标进行漏洞扫描 | `PortScan`, `EXECMD` |
...
| nmap | 全端口扫描与服务识别（LLM 驱动端口发现） |
| xray | Web 漏洞自动化扫描 |
| netcat (nc) | 网络连接与端口探测（可选） |
| Playwright | 浏览器自动化（可选） |
...
├── install_tools.ps1             # Windows 系统扫描工具一键安装脚本（nmap/xray/nc等）
├── frontend/                     # Web前端界面
│   ├── index.html               # 主页面（SPA）
│   ├── style.css                # 样式（赛博朋克风格）
...
│   ├── knowledge.py             # 服务知识库加载（端口映射/CVE模式/LLM驱动nmap端口发现）
...
├── xray/                         # xray扫描器（需手动放置）
│   └── xray_windows_amd64.exe   # xray可执行文件
└── README.md
```
...
| nmap | ✅ 必需 | 全端口扫描与服务识别（LLM 驱动的端口发现核心依赖） |
| xray | ⭐ 推荐 | Web 漏洞自动化扫描（Scan Agent 核心依赖） |
| netcat (nc) | ⚡ 可选 | 网络连接与端口探测 |
| curl / wget | ⚡ 可选 | HTTP 请求与文件下载（通常系统自带） |
...
#### 2. 安装系统扫描工具（nmap / xray 等）

项目提供了 Windows PowerShell 一键安装脚本，自动检测系统环境并安装所有需要的扫描工具：
...
| 工具 | 用途 | 必要性 |
|------|------|--------|
| **nmap** | 全端口扫描与服务识别，LLM 驱动的端口发现核心引擎 | ✅ 必需 |
| **xray** | Web 漏洞自动化扫描（Scan Agent 核心依赖） | ⭐ 推荐 |
| **netcat (ncat)** | 网络连接与端口探测工具（nmap 自带 ncat） | ⚡ 可选 |
| **curl** | HTTP 请求工具（Windows 10+ 系统自带） | ⚡ 可选 |

<details>
<summary>📋 手动安装方式（如果脚本不适用）</summary>

**通过 winget 安装（推荐，Windows 10 1709+ 自带）：**
```powershell
...
- 下载 `.exe` 安装包，安装时勾选 Npcap 驱动

**xray 手动安装：**
```powershell
mkdir xray
# 从 https://github.com/chaitin/xray/releases 下载 xray_windows_amd64.exe.zip
# 解压后放入 xray\ 目录
```
...
# 1. 从 https://www.docker.com/products/docker-desktop/ 下载 Docker Desktop 安装包
# 2. 运行安装程序，安装完成后重启电脑
# 3. 启动 Docker Desktop
...
> **注意**: Windows 上需要启用 WSL 2 或 Hyper-V 后端。安装 Docker Desktop 时会自动引导配置。
...
当知识库未收录目标服务时，系统会自动启用**LLM 驱动端口发现**机制：LLM 自主生成 nmap 扫描命令（如 `nmap -sS -p- --min-rate 3000 -T4`），执行后分析结果，再决定下一步扫描策略（服务识别、NSE 脚本探测等），循环最多 10 轮直到完整输出端口映射表。
